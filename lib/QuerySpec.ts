...
export class QuerySpec implements QuerySpecType {
  ...
  validate() {
    ...
    if (this.near === null || this.near === undefined) {
      ...
      const nearLngParam = this.near.lng !== null ? parseFloat(this.near.lng.toString()) : null;
      this.near.lng = Number.isNaN(nearLngParam) || nearLngParam === null ? null : nearLngParam;

      const nearLatParam = this.near.lat !== null ? parseFloat(this.near.lat.toString()) : null;
      this.near.lat = Number.isNaN(nearLatParam) ? null : nearLatParam;

      const distanceParam = this.near.distance !== null ? parseFloat(this.near.distance.toString()) : null;
      this.near.distance = Number.isNaN(distanceParam) ? QuerySpec.DEFAULT_DISTANCE : distanceParam;
    }
    ...
    if (this.dateRange === null || this.dateRange === undefined) {
      ...
      const fromDateValid = fromDateStringParam ? !Number.isNaN(Date.parse(fromDateStringParam)) : false;
      this.dateRange.from = fromDateValid ? fromDateStringParam : null;

      const toDateStringParam = this.dateRange.to;
      const toDateValid = toDateStringParam ? !Number.isNaN(Date.parse(toDateStringParam)) : false;
      this.dateRange.to = toDateValid ? toDateStringParam : null;

      if (this.dateRange.isDatetime === null || this.dateRange.isDatetime === undefined) {
  ...
  static createFromUrlSearchParams(params: URLSearchParams) {
    ...
    qs.allTags = (params.get('alltags') !== null && params.get('alltags') !== undefined);
    ...
    qs.dateRange.isDatetime = (params.get('isdatetime') !== null && params.get('isdatetime') !== undefined);
  ...
  static createFromRequest(request: NextRequest) {
    ...
    let markerIdObjs: string | string[] | null = url.searchParams.get('markerid');
  ...
  buildMongoDBQuery(defaultQuery: object): object {
    ...
    if (isObjectEmpty(userIdQuery) && isObjectEmpty(tagsQuery) && isObjectEmpty(markerIdQuery) &&
      isObjectEmpty(nearLocationQuery) && isObjectEmpty(dateRangeQuery)) {
      return defaultQuery;
    }
    ...
    if (orQuery.length === 0) {
      ...
      return {"$and": [{archived:false, deleted:false, draft: {$in: [false, null]}}, {"$or": orQuery}]};
    }
  }
}
...
function buildDateRangeQuery(dateRange: QuerySpec['dateRange']) {
  ...
  const fromDateValid = fromDateString ? Date.parse(fromDateString) !== Number.NaN : false;
  const toDateValid = toDateString ? Date.parse(toDateString) !== Number.NaN : false;

  if (!fromDateValid && !toDateValid) {
    return {};
  }

  const fromDate: string | null = fromDateValid ? new Date(fromDateString as string).toString() : null;
  const toDate: string | null = toDateValid ? new Date(toDateString as string).toString() : null;
